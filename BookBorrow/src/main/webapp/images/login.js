					...
					success : function(data) {
						if (data == "true") {
							...
							if ($('#user').val() == ""
								...
								$('#userCue').html(
										"<font color='green'>该用户名可用！</font>");
							}
						}
					}
				});
				// $.get("userisexist?name=" + this.val(), function(data) {
				// alert(data);
				// if (data == true) {
				// $('#ajaxres').html("该用户已经注册！");
				// } else {
				// $('#ajaxres').html("该用户可用");
				// }
				// });
							...
							.click(
									function() {
										...
										if (!sqq.test($('#qq').val())
											...
											$('#userCue')
													.html(
															"<font color='red'><b>×电话号码格式不正确</b></font>");
											return false;
										} else {
											$('#qq').css({
												border : "1px solid #D7D7D7",
